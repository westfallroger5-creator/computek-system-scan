...
The remote-access and post-scam scanners suppress the generic user-profile warning for Teams and OneDrive only when the executable is in an expected Microsoft folder and has a valid Microsoft digital signature. A lookalike name, invalid or missing signature, unexpected folder, or suspicious command line is still reported.
