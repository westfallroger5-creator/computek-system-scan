...
Assert-AppTest ($moduleSource -match 'McAfee\\Proxy\\data' -and $postScamSource -match 'Known Gen Digital security-product hosts entries' -and $postScamSource -match 'Severity ''Informational'' -Name "Common utility prefetch:' -and $postScamSource -match 'third-party antivirus is registered') 'Known McAfee protection, Gen Digital hosts entries, common archive Prefetch, and inactive Defender under third-party antivirus do not create false failures'
...
Assert-AppTest ($remoteSource -match 'Remove-CandidateFirewallRules' -and $remoteSource -match 'Remove-NetFirewallRule' -and $remoteSource -match 'RemainingFirewallRules') 'Selected remote products have their exact associated firewall rules removed and verified'
...
try {
    ...
    Assert-AppTest ($assembly.GetName().Version.ToString() -eq '1.5.9.0') 'Built EXE reports version 1.5.9.0'
