...
Assert-AppTest ($postScamSource -match 'New-CompuTekPostScamHtmlReport' -and $postScamSource -match 'PostScamReport\.html' -and $postScamSource -match 'Opening the easy-to-read report' -and $postScamSource -match 'Start-Process -FilePath \$htmlReportPath') 'Post-scam collection creates and automatically opens a self-contained easy-to-read HTML summary'
...
try {
    ...
    Assert-AppTest ($assembly.GetName().Version.ToString() -eq '1.4.22.0') 'Built EXE reports version 1.4.22.0'
