...
- Consolidates repeated evidence into `ActionableFindings.txt` and a clean, offline `PostScamReport.html`. The HTML report groups items by technician priority, separates warning groups from supplemental inventory and collection gaps, and is opened by the Windows application when the scan finishes. An **Open last report** button remains available if the browser was closed or Windows blocked the first attempt. The application shows at most eight groups so its live output remains readable.
