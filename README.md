...
- Inspects machine and loaded-user uninstall entries, all-user AppX packages, services, running processes, active connections, Run keys, scheduled tasks, the all-users Startup folder, every user profile's Startup/AppData/Desktop/Downloads folders, ProgramData, and Windows Temp. If the all-user Store query fails, it still checks the current user's packages and marks the scan incomplete so the gap cannot be mistaken for a clean result.
