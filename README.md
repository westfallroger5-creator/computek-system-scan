...
- Always offers technician-reviewed removal when findings exist. Numbered agents can be classified in batches such as `KEEP 1,3-5` and `REMOVE 2,6-8`; every number must be classified and the proposed list confirmed. Type `OPEN 1` (or a range) at either decision prompt to show the detected downloaded installer or portable remote-tool file in File Explorer; installed Program Files folders are not opened by this command. No remediation begins until the technician also types `APPLY REMOVALS`.
